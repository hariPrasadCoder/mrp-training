...
export const enrollmentStatus = pgEnum("enrollment_status", ["invited", "onboarding", "active", "paused", "completed"]);
export const releaseStatus = pgEnum("release_status", ["locked", "available", "credited", "completed"]);
export const blockType = pgEnum("block_type", ["video", "text", "link", "checklist", "booking", "submission"]);
export const submissionStatus = pgEnum("submission_status", ["draft", "submitted", "changes_requested", "approved", "standout"]);
export const bookingKind = pgEnum("booking_kind", ["saturday_checkpoint", "office_hour"]);
export const bookingStatus = pgEnum("booking_status", ["pending", "booked", "rescheduled", "cancelled", "completed", "no_show"]);
...
}, (table) => [uniqueIndex("profiles_auth_user_id_unique").on(table.authUserId), uniqueIndex("profiles_email_unique").on(table.email)]);

export const learnerChecklist = pgTable("learner_checklist", {
...
export const programs = pgTable("programs", {
  ...
  createdAt: timestamp("created_at", { withTimezone: true }).defaultNow().notNull(),
}, (table) => [uniqueIndex("program_name_version_unique").on(table.name, table.version)]);
...
}, (table) => [uniqueIndex("program_week_number_unique").on(table.programId, table.weekNumber)]);
...
export const contentBlocks = pgTable("content_blocks", {
  ...
  content: jsonb("content").$type<{ videoId?: string; url?: string; body?: string; durationMinutes?: number }>().notNull().default({}),
...
export const enrollments = pgTable("enrollments", {
  id: uuid("id").defaultRandom().primaryKey(),
  learnerId: uuid("learner_id").notNull().references(() => profiles.id, { onDelete: "cascade" }),
  ...
  createdAt: timestamp("created_at", { withTimezone: true }).defaultNow().notNull(),
}, (table) => [uniqueIndex("learner_program_unique").on(table.learnerId, table.programId), index("enrollments_status_idx").on(table.status)]);

export const weekReleases = pgTable("week_releases", {
...
export const submissions = pgTable("submissions", {
  id: uuid("id").defaultRandom().primaryKey(),
  enrollmentId: uuid("enrollment_id").notNull().references(() => enrollments.id, { onDelete: "cascade" }),
  weekId: uuid("week_id").notNull().references(() => programWeeks.id),
  version: integer("version").notNull().default(1),
  ...
  createdAt: timestamp("created_at", { withTimezone: true }).defaultNow().notNull(),
}, (table) => [index("submissions_review_queue_idx").on(table.status, table.submittedAt)]);

export const feedback = pgTable("feedback", {
  id: uuid("id").defaultRandom().primaryKey(),
  submissionId: uuid("submission_id").notNull().references(() => submissions.id, { onDelete: "cascade" }),
  coachId: uuid("coach_id").notNull().references(() => profiles.id),
  result: submissionStatus("result").notNull(),
  summary: text("summary").notNull(),
  rubric: jsonb("rubric").$type<Array<{ criterion: string; score: number; note: string }>>().notNull().default([]),
  createdAt: timestamp("created_at", { withTimezone: true }).defaultNow().notNull(),
});
...
export const bookings = pgTable("bookings", {
  ...
  createdAt: timestamp("created_at", { withTimezone: true }).defaultNow().notNull(),
  updatedAt: timestamp("updated_at", { withTimezone: true }).defaultNow().notNull(),
}, (table) => [uniqueIndex("bookings_cal_uid_unique").on(table.calBookingUid), index("bookings_start_idx").on(table.startsAt)]);
