...
export async function PUT(request: Request) {
  if (isRateLimited(request, "learner-progress", 120))
  ...
  if (typeof body?.blockId !== "string" || typeof body?.completed !== "boolean")
    return NextResponse.json(
  ...
  const block = (await getProgramContent()).find(
    (item) => item.id === body.blockId,
  );
  const release =
    block &&
    workspace.releases.find((item) => item.weekNumber === block.weekNumber);
  if (!block || !release || release.status === "locked")
    return NextResponse.json(
