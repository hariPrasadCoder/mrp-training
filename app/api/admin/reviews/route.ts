...
export async function POST(request: Request) {
  ...
  if (typeof body?.submissionId !== "string" || !["changes_requested", "approved", "standout"].includes(result) || !summary) return NextResponse.json({ error: "Choose a result and add feedback." }, { status: 400 });
  await reviewSubmission(body.submissionId, admin.id, { result, summary });
